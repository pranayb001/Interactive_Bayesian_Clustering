...
%           end
%           r = q(:,N);
%           fun = @(r)beta*I_q_theta_thetaSNew( S, K,  [q(:,1:N-1),r], theta_old , X, clst_rej, clst_acc ) + alpha * KLDivNew(P_h_given_x',[q(:,1:N-1),r],K,N);
%           r = fminunc(fun,r);
%           q(:,N)=r;
%           iterCD = iterCD + 1;
%       end
...
%        cellsz = cellfun(@size,co_var_mat,'uni',false);
%        cellsz{:}
%          disp('co_var_mat');
%          disp(size(co_var_mat));
%          disp('q');
%          disp(size(q));
%         disp('X');
%         disp(size(X));
